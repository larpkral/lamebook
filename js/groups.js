...
function avatarEl(url, name, size = 40, radius = '50%') {
  if (url) return `<img src="${url}" alt="${name}" style="width:${size}px;height:${size}px;border-radius:${radius};object-fit:cover">`;
  const init = (name || '?').split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase();
  return `<div class="avatar-placeholder" style="width:${size}px;height:${size}px;font-size:${size*0.38}px;border-radius:${radius}">${init}</div>`;
}
...
export async function renderGroups(currentUser, currentProfile, groupId) {
  ...
  wrap.className = 'groups-layout';
  container.appendChild(wrap);

  // Left sidebar
  const sidebar = document.createElement('div');
  sidebar.className = 'groups-sidebar card';
  sidebar.style.padding = '16px';
  sidebar.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      <h2 style="font-size:24px;font-weight:700">Groups</h2>
  ...
  allGroups.forEach(g => {
    ...
    item.className = 'group-item' + (groupId === g.id ? ' active' : '');
    item.dataset.id = g.id;
    item.innerHTML = `
      ${g.cover_url
        ? `<img src="${g.cover_url}" style="width:56px;height:56px;border-radius:8px;object-fit:cover">`
        : `<div style="width:56px;height:56px;border-radius:8px;background:linear-gradient(135deg,#1877f2,#42b72a);flex-shrink:0"></div>`}
      <div class="group-item-info">
        <div class="group-item-name">${escapeHtml(g.name)}</div>
        <div class="group-item-meta">Group</div>
      </div>`;
    item.addEventListener('click', () => {
      sidebar.querySelectorAll('.group-item').forEach(el => el.classList.remove('active'));
  ...
  if (groupId) {
    ...
    main.innerHTML = '<div class="empty-state"><div class="icon">👥</div><p>Select a group from the list.</p></div>';
  } else {
    main.innerHTML = '<div class="empty-state"><div class="icon">👥</div><p>Create or join a group to get started.</p></div>';
  }
}
...
async function renderGroupFeed(groupId, currentUser, currentProfile, container) {
  ...
  if (group.cover_url) {
    coverEl.innerHTML = `<img src="${group.cover_url}" class="group-cover" alt="cover">`;
  } else {
    coverEl.innerHTML = `<div class="group-cover-placeholder"></div>`;
  }
  coverEl.innerHTML += `
    <div style="padding:16px">
      <h2 style="font-size:28px;font-weight:700;margin-bottom:4px">${escapeHtml(group.name)}</h2>
      <div style="color:var(--text2);font-size:14px;margin-bottom:12px">🔒 Private group · ${memberCount || 0} members</div>
      ${group.description ? `<p style="font-size:15px;margin-bottom:12px">${escapeHtml(group.description)}</p>` : ''}
  ...
  createCard.style.padding = '12px 16px';
  createCard.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px">
      ${avatarEl(currentProfile?.avatar_url, currentProfile?.full_name)}
      <button class="create-post-input" id="open-group-post" style="text-align:left">Write something to the group…</button>
    </div>`;
  ...
  for (const post of posts) {
    // Build a post-card-like element for group posts
    const card = await buildGroupPostCard(post, currentUser, currentProfile);
    postsWrap.appendChild(card);
  }
}

async function buildGroupPostCard(post, currentUser, currentProfile) {
  // Reuse feed's buildPostCard logic with group_posts table awareness
  ...
  card.innerHTML = `
    <div class="post-header">
      ${author?.avatar_url
        ? `<img src="${author.avatar_url}" alt="${author.full_name}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;cursor:pointer" onclick="window.location.hash='#profile/${author.id}'">`
        : `<div class="avatar-placeholder" style="width:40px;height:40px;font-size:15px;cursor:pointer">${(author?.full_name||'?')[0].toUpperCase()}</div>`}
      <div class="post-header-info">
        <div class="post-author" onclick="window.location.hash='#profile/${author?.id}'">${author?.full_name || 'User'}</div>
    ...
    <div class="post-actions">
      <button class="post-action-btn"><span class="icon">👍</span> Like</button>
      <button class="post-action-btn"><span class="icon">💬</span> Comment</button>
    </div>`;
  return card;
}
...
function openGroupPostModal(groupId, currentUser, currentProfile, onPost) {
  ...
  modal.innerHTML = `
    <div class="modal">
      <div class="modal-header"><h2>Create post</h2><button class="modal-close">✕</button></div>
      <div class="modal-body">
        <div id="gp-error" style="color:#c0392b;font-size:13px;margin-bottom:8px;display:none"></div>
        <textarea placeholder="Write something…" style="width:100%;border:none;font-size:20px;font-family:var(--font);resize:none;outline:none;color:var(--text);min-height:100px"></textarea>
        <img id="gp-preview" style="width:100%;border-radius:8px;margin-top:8px;display:none" src="" alt="">
      ...
      <div class="modal-footer"><button class="btn-primary" id="gp-submit" style="justify-content:center">Post</button></div>
    </div>`;
  modal.querySelector('.modal-close').addEventListener('click', () => modal.remove());
  modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
  const imgInput = modal.querySelector('#gp-img-input');
  imgInput.addEventListener('change', () => { if (imgInput.files[0]) { modal.querySelector('#gp-preview').src = URL.createObjectURL(imgInput.files[0]); modal.querySelector('#gp-preview').style.display = 'block'; } });
...
function openCreateGroupModal(currentUser, onCreated) {
  ...
  modal.innerHTML = `
    <div class="modal">
      <div class="modal-header"><h2>Create group</h2><button class="modal-close">✕</button></div>
      <div class="modal-body">
        <div id="cg-error" style="color:#c0392b;font-size:13px;margin-bottom:8px;display:none"></div>
        <label style="font-weight:600;display:block;margin-bottom:4px;font-size:14px">Group name *</label>
        <input id="cg-name" type="text" placeholder="Enter group name" style="width:100%;border:1px solid var(--border);border-radius:8px;padding:10px;font-size:15px;margin-bottom:12px;font-family:var(--font);outline:none">
        <label style="font-weight:600;display:block;margin-bottom:4px;font-size:14px">Description</label>
        <textarea id="cg-desc" placeholder="What's this group about?" style="width:100%;border:1px solid var(--border);border-radius:8px;padding:10px;font-size:15px;resize:none;height:80px;font-family:var(--font);outline:none;margin-bottom:12px"></textarea>
        <label style="font-weight:600;display:block;margin-bottom:4px;font-size:14px">Cover photo</label>
        <label style="cursor:pointer">
          <div id="cg-cover-label" style="border:2px dashed var(--border);border-radius:8px;padding:20px;text-align:center;color:var(--text2);font-size:14px">📷 Click to upload cover photo</div>
          <input type="file" id="cg-cover-input" accept="image/*" style="display:none">
        </label>
        <img id="cg-cover-preview" style="width:100%;border-radius:8px;margin-top:8px;display:none" src="" alt="">
      </div>
      <div class="modal-footer"><button class="btn-primary" id="cg-submit" style="justify-content:center">Create group</button></div>
    </div>`;
  modal.querySelector('.modal-close').addEventListener('click', () => modal.remove());
  modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
