...
function avatarEl(url, name, size = 40) {
  ...
  return `<div class="av-ph" style="width:${size}px;height:${size}px;font-size:${Math.round(size * 0.38)}px">${initials}</div>`;
}
...
export async function renderFriends(currentUser, currentProfile) {
  ...
  if (incoming?.length) {
    ...
    section.style.cssText = 'padding:16px;max-width:100%';
    section.innerHTML = `<div class="sec-head"><h3>Friend Requests</h3></div>`;
    const grid = document.createElement('div');
    grid.className = 'friends-grid';
    incoming.forEach(req => {
      const p = req.profiles;
      const card = document.createElement('div');
      card.className = 'friend-tile';
      card.innerHTML = p.avatar_url
        ? `<img src="${p.avatar_url}" alt="${esc(p.full_name)}">`
        : `<div class="ft-ph">${(p.full_name||'?')[0].toUpperCase()}</div>`;
      card.innerHTML += `
        <div class="ft-body"><div class="ft-name">${esc(p.full_name)}</div></div>
        <div class="ft-btns">
  ...
  if (suggestions?.length) {
    ...
    section.className = 'card';
    section.style.cssText = 'padding:16px;max-width:100%';
    section.innerHTML = `<div class="sec-head"><h3>People You May Know</h3></div>`;
    const grid = document.createElement('div');
    grid.className = 'friends-grid';
  ...
  if (friendIds.length) {
    ...
    section.className = 'card';
    section.style.cssText = 'padding:16px;max-width:100%';
    section.innerHTML = `<div class="sec-head"><h3>All Friends</h3></div>`;
    const grid = document.createElement('div');
    grid.className = 'friends-grid';
    (friends || []).forEach(p => {
      const card = document.createElement('div');
  ...
  if (!wrap.children.length) {
    wrap.innerHTML = '<div class="empty-state"><div class="es-icon">👥</div><p>No friends or suggestions yet.</p></div>';
  }
}
